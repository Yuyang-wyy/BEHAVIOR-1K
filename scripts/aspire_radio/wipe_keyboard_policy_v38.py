...
if not HELD:
    ...
    if again is not None:
        ...
        ACROSS = np.array([-KEYBOARD_AXIS[1], KEYBOARD_AXIS[0]])
        print("keyboard re-detected at %s top %.3f" % (np.round(KEYBOARD_HOME, 3), KEYBOARD_TOP))
    else:
    ...
    if not refreshed:
        ...
        for index in range(11):
            ...
            view = get_observation()
            remember(view)
            again = keyboard_near(world_points(view), LEVEL, KEYBOARD_HOME[:2], get_robot_position()[0][:2])
            if again is not None:
                ...
                print("keyboard found on the stance search at %s" % np.round(KEYBOARD_HOME, 3))
                refreshed = True
        if refreshed:
            ACROSS = np.array([-KEYBOARD_AXIS[1], KEYBOARD_AXIS[0]])
